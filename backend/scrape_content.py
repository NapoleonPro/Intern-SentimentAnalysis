...
def get_empty_articles():
    """Mengambil daftar artikel yang kolom content-nya masih kosong"""
...
def update_content(article_id, content_text):
    """Update database dengan isi berita yang baru discrape"""
    with engine.connect() as conn:
...
def scrape_body():
    ...
    for index, row in enumerate(articles):
        ...
        try:
            ...
            # Mengidentifikasi elemen konten utama.
            # Mencari div dengan id 'single-article-text'.
            content_div = soup.find('div', id='single-article-text')

            # Jika ID tidak ditemukan, coba cari berdasarkan class.
            if not content_div:
                content_div = soup.find('div', class_='single-article-text')

            if content_div:
                # Ambil semua teks dalam tag paragraph <p>
                paragraphs = content_div.find_all('p')
                # Gabungkan jadi satu string panjang
                full_text = "\n\n".join([p.get_text(strip=True) for p in paragraphs])
                
                # Bersihkan sedikit (hapus teks "Baca Juga" atau iklan jika tertangkap)
                if "Baca Juga" in full_text:
                    full_text = full_text.split("Baca Juga")[0] # Potong bagian bawah iklan
        ...
        # Jeda untuk menghindari pemblokiran IP.
