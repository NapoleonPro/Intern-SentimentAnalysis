...
def get_sentiment_batch(berita_list):
    """
    Mengirim BANYAK berita sekaligus dalam satu prompt agar hemat request.
    berita_list: [(id, text), (id, text), ...]
    """
    
    # Rakit Prompt Raksasa
...
def run_analysis():
    ...
    # Pecah jadi chunk (kelompok-kelompok kecil)
    ...
    for chunk in chunks:
        ...
        if results:
            ...
            # --- PERBAIKAN UTAMA: Gunakan Transaksi ---
            with engine.begin() as conn: # .begin() akan otomatis commit/rollback
                for res in results:
                    # Validasi dan mapping label
        ...
        # Jeda Wajib (Rate Limiter)
        # Aman: Jeda 4 detik per batch (15 req/menit).
        time.sleep(4) 

    print(f"🏁 Selesai! Total berita dianalisis: {total_processed}")
