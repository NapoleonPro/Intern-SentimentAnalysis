...
CUTOFF_DAYS = 90  # Ambil berita 3 bulan terakhir
...
def check_date(date_str):
    ...
    Return:
      0 = Tanggal OK (Baru)
      1 = Tanggal Kadaluarsa (Skip)
      2 = Error parsing (Lanjut aja)
    """
...
def run_scraper():
    ...
    while keep_going:
        ...
        try:
            ...
            if not articles:
                print("   ⚠️ Tidak ada artikel. Selesai.")
                break
            
            # Counter untuk cek apakah halaman ini isinya berita lama semua
            ...
            for item in articles:
                ...
                if date_status == 1: 
                    # Jika tua, SKIP saja (jangan stop)
                    # print(f"   [SKIP - Tua] {pub_date[:10]} - {title[:20]}...")
            ...
            # --- PENENTUAN BERHENTI ---
            # Kita baru berhenti kalau SEMUA artikel di halaman ini ternyata berita lama.
            # Artinya kita sudah masuk zona arsip tua sepenuhnya.
            if old_articles_on_page == len(articles):
                print(f"   🛑 Seluruh halaman {page} berisi berita lama. Stop Scraping.")
                keep_going = False
                break
            
            # Kalau masih ada 1 pun berita baru nyempil, lanjut ke halaman depan!
            page += 1
            time.sleep(1) 
